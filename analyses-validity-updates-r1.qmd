...
# Follow-up analyses: 8-month and 12-month centering 

To explore persistence of effects, we also re-ran the models for substance use consequences, depressive symptoms, and internalized heterosexism where we re-centered the assessment time point. 

```{r}
m4.6_hamd <- with(imp, 
               geeglm(HAMD_sum ~ Site + CMHT + MHgen + Medication + WAI_avg_c + I(Assessment^2) + Average_Affirm_c + Assessment*Average_Affirm_c,
                     id = ParticipantID,
                     family = gaussian,
                     corstr = "ar1")) 
...
```{r}
m3.4_sip <- with(imp, 
               geeglm(SIP_sum ~ Site + CMHT + MHgen + Medication + WAI_avg_c + Assessment_c_8m + Average_Affirm_c + Assessment_c_8m*Average_Affirm_c,
                     id = ParticipantID,
                     family = poisson,
                     corstr = "ar1")) 
...
```{r}
m4.4_hamd <- with(imp, 
               geeglm(HAMD_sum ~ Site + CMHT + MHgen + Medication + WAI_avg_c + I(Assessment_c_8m^2) + Average_Affirm_c + Assessment_c_8m*Average_Affirm_c,
                     id = ParticipantID,
                     family = gaussian,
                     corstr = "ar1")) 
...
```{r}
m7.4_ihs <- with(imp, 
               geeglm(IHS_mean ~ Site + CMHT + MHgen + Medication + WAI_avg_c + Assessment_c_8m + Average_Affirm_c + Assessment_c_8m*Average_Affirm_c,
                     id = ParticipantID,
                     family = gaussian,
                     corstr = "ar1")) 
...
```{r}
m3.5_sip <- with(imp, 
               geeglm(SIP_sum ~ Site + CMHT + MHgen + Medication + WAI_avg_c + I(Assessment_c_12m^2) + Average_Affirm_c + Assessment_c_12m*Average_Affirm_c,
                     id = ParticipantID,
                     family = poisson,
                     corstr = "ar1")) 
...
```{r}
m4.5_hamd <- with(imp, 
               geeglm(HAMD_sum ~ Site + CMHT + MHgen + Medication + WAI_avg_c + Assessment_c_12m + Average_Affirm_c + Assessment_c_12m*Average_Affirm_c,
                     id = ParticipantID,
                     family = gaussian,
                     corstr = "ar1")) 
...
```{r}
m7.5_ihs <- with(imp, 
               geeglm(IHS_mean ~ Site + CMHT + MHgen + Medication + WAI_avg_c + Assessment_c_12m + Average_Affirm_c + Assessment_c_12m*Average_Affirm_c,
                     id = ParticipantID,
                     family = gaussian,
                     corstr = "ar1")) 
...
#### INTERACTION PROBING - TO DO 

Get a J-N type of estimate where we can see each value of the LADI and a test of whether the association between LADI and instantaneous linear change is significantly different from zero

```{r}
slopes(m5.4_bai, variables = "Assessment_c", newdata = datagrid(Assessment_c = c(0, 1, 4)))

slopes(m5.4_bai, variables = "Average_Affirm_c", newdata = datagrid(apps = c(-1.2, 0, 1.2), Assessment_c = c(0, 1, 2, 3))) 

avg_slopes(m5.4_bai, variables = "Assessment_c", by = "Average_Affirm_c", newdata = datagrid(Average_Affirm_c = c(-1.2, 0, 1.2), Assessment_c = c(0, 1, 2, 3)))
...
```{r}
avg_slopes(m5.4_bai, variables = "Assessment_c", by = "Average_Affirm_c", newdata = datagrid(Average_Affirm_c = c(-1.2, 1.2)))
```
