...
Load full dataset to use for reporting demographics, calculating reliability, and pulling variables not included in prior apps dataset when initially requested. [code not displayed for peer review]
...
# create a variable denoting who was included in this study vs not for descriptive comparisons, put in new dataframe 
comp <- full |> 
  mutate(analytic_yn = if_else(ParticipantID %in% id$ParticipantID, T, F))
...
# Analytic vs. full community-based sample comparisons in outcomes 
...
```{.r .cell-code}
# first, select only the relevant variables here
comp <- comp |> 
  select(ParticipantID, Site, Condition, Assessment, analytic_yn, Sx_CASriskacts, AUDIT_sum, SIP_sum, HAMD_sum, BAI_sum, SIDAS_sum, IHS_mean, RS_mean, SOC_concealment_mean, LGBIS_identaffirm) |> 
  # select only the other community participants & BL assessment 
  filter(Condition == 2, Assessment == 0) 

# create a function to run the comparison for all the outcomes at once
output <- comp %>% 
  select(ParticipantID, Site, Condition, Assessment, analytic_yn, Sx_CASriskacts, AUDIT_sum, SIP_sum, HAMD_sum, BAI_sum, SIDAS_sum, IHS_mean, RS_mean, SOC_concealment_mean, LGBIS_identaffirm) |> 
  # select only the other community participants & BL assessment 
  filter(Condition == 2, Assessment == 0) |> 
  select(ParticipantID, Sx_CASriskacts, AUDIT_sum, SIP_sum, HAMD_sum, BAI_sum, SIDAS_sum, IHS_mean, RS_mean, SOC_concealment_mean, LGBIS_identaffirm, analytic_yn) %>% 
  pivot_longer(c(Sx_CASriskacts, AUDIT_sum, SIP_sum, HAMD_sum, BAI_sum, SIDAS_sum, IHS_mean, RS_mean, SOC_concealment_mean, LGBIS_identaffirm),
               names_to = "variable",
               values_to = "outcome") %>% 
  ...
  map_dfr(.f = function(df) {
    lm(outcome ~ 1 + analytic_yn, data = df) %>% 
      tidy() %>% 
...
output |> 
  ...
  # for ease here, remove the intercept b/c we don't need this 
  filter(term == "analytic_ynTRUE")
```
...
# A tibble: 10 × 6
 ...
 1 AUDIT_sum            analytic_ynTRUE    0.878     1.44      0.61    0.544
 2 BAI_sum              analytic_ynTRUE    2.29      2.35      0.974   0.333
 3 HAMD_sum             analytic_ynTRUE    0.796     1.37      0.583   0.562
 4 IHS_mean             analytic_ynTRUE   -0.071     0.143    -0.496   0.621
 5 LGBIS_identaffirm    analytic_ynTRUE   -0.093     0.251    -0.369   0.713
 6 RS_mean              analytic_ynTRUE    0.545     1.56      0.35    0.727
 7 SIDAS_sum            analytic_ynTRUE    2.07      1.13      1.83    0.07 
 8 SIP_sum              analytic_ynTRUE   -0.963     0.883    -1.09    0.278
 9 SOC_concealment_mean analytic_ynTRUE   -0.197     0.153    -1.29    0.2  
10 Sx_CASriskacts       analytic_ynTRUE   -2.74      1.86     -1.47    0.144
```
...
```{.r .cell-code}
# remove to clean up environment
rm(output)
```
:::
...
```{.r .cell-code}
# separate out variable & take first observation so data is no longer in long format - ladi scores did not vary by timepoint
ladi <- apps_outcome |> 
  select(ParticipantID, Early_Affirm, Mid_Affirm) |> 
  group_by(ParticipantID) |> 
  slice_head() |> 
  ungroup()

t.test(ladi$Early_Affirm, ladi$Mid_Affirm, paired = T)
```
...
data:  ladi$Early_Affirm and ladi$Mid_Affirm
t = 0.23594, df = 56, p-value = 0.8143
...
Early_Affirm    1 57 1.40 1.56      1    1.17 1.48   0   7     7 1.48     2.53
Mid_Affirm      2 57 1.35 1.38      1    1.16 1.48   0   5     5 1.03     0.20
               se
Early_Affirm 0.21
Mid_Affirm   0.18
```
...
```{.r .cell-code}
# remove ladi dataframe from environment
rm(ladi)
```
:::
...
For outcome analyses, we should center assessment around the post-treatment outcome scores. This would mean that the intercept would reflect the mean outcome variable score post-treatment/at 4MFU. 
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-30-2.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-30-3.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-30-4.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-30-5.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-30-6.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-30-7.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-30-8.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-30-9.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-30-10.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-30-11.png){width=672}
:::
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-31-3.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-31-4.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-31-5.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-31-6.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-31-7.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-31-8.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-31-9.png){width=672}
:::
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-32-2.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-32-3.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-32-4.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-32-5.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-32-6.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-32-7.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-32-8.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-32-9.png){width=672}
:::
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-33-1.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-33-2.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-33-3.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-33-4.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-33-5.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-33-6.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-33-7.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-33-8.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-33-9.png){width=672}
:::
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-34-1.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-34-2.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-34-3.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-34-4.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-34-5.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-34-6.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-34-7.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-34-8.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-34-9.png){width=672}
:::
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-36-2.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-36-3.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-36-4.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-36-5.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-36-6.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-36-7.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-36-8.png){width=672}
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-36-9.png){width=672}
:::
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-38-1.png){width=672}
:::
:::
...
::: {.cell-output-display}
![](analyses-basic_files/figure-html/unnamed-chunk-39-1.png){width=672}
:::
:::
...
Specifically for the WAI & LADI:


::: {.cell}

```{.r .cell-code}
# since only 1 score for alliance and LADI, reduce dataset to n=57
ladi <- apps_outcome |> 
  group_by(ParticipantID) |> 
  slice_head() |> 
  ungroup() |> 
  select(ParticipantID, contains("affirm"), contains("WAI"))

cor.test(ladi$WAI_early, ladi$Early_Affirm) 
```
...
data:  ladi$WAI_early and ladi$Early_Affirm
t = 0.82886, df = 54, p-value = 0.4108
...
```{.r .cell-code}
cor.test(ladi$WAI_mid, ladi$Mid_Affirm) 
```
...
data:  ladi$WAI_mid and ladi$Mid_Affirm
t = 1.2238, df = 55, p-value = 0.2262
alternative hypothesis: true correlation is not equal to 0
95 percent confidence interval:
 -0.1020791  0.4061557
sample estimates:
...
```{.r .cell-code}
cor.test(ladi$WAI_avg, ladi$Average_Affirm)
```
...
data:  ladi$WAI_avg and ladi$Average_Affirm
t = 1.1246, df = 55, p-value = 0.2656
alternative hypothesis: true correlation is not equal to 0
95 percent confidence interval:
 -0.1151378  0.3950619
sample estimates:
...
other attached packages:
 [1] broom_1.0.8        lmerTest_3.1-3     lme4_1.1-37        Matrix_1.7-3      
 [5] gridExtra_2.3      psych_2.5.3        see_0.11.0         report_0.6.1      
 [9] parameters_0.26.0  performance_0.13.0 modelbased_0.10.0  insight_1.3.0     
[13] effectsize_1.0.0   datawizard_1.0.2   correlation_0.8.7  bayestestR_0.16.0 
[17] easystats_0.7.4    haven_2.5.4        sjmisc_2.8.10      lubridate_1.9.4   
[21] forcats_1.0.0      stringr_1.5.1      dplyr_1.1.4        purrr_1.0.4       
[25] readr_2.1.5        tidyr_1.3.1        tibble_3.2.1       ggplot2_3.5.2     
[29] tidyverse_2.0.0   

loaded via a namespace (and not attached):
 [1] sjlabelled_1.2.0    tidyselect_1.2.1    farver_2.1.2       
 [4] fastmap_1.2.0       digest_0.6.37       timechange_0.3.0   
 [7] lifecycle_1.0.4     magrittr_2.0.3      compiler_4.5.0     
[10] rlang_1.1.6         tools_4.5.0         utf8_1.2.4         
[13] yaml_2.3.10         knitr_1.50          labeling_0.4.3     
[16] htmlwidgets_1.6.4   mnormt_2.1.1        RColorBrewer_1.1-3 
[19] withr_3.0.2         numDeriv_2016.8-1.1 grid_4.5.0         
[22] future_1.40.0       globals_0.17.0      scales_1.4.0       
[25] MASS_7.3-65         cli_3.6.5           rmarkdown_2.29     
[28] crayon_1.5.3        reformulas_0.4.0    generics_0.1.3     
[31] rstudioapi_0.17.1   tzdb_0.5.0          minqa_1.2.8        
[34] splines_4.5.0       parallel_4.5.0      vctrs_0.6.5        
[37] boot_1.3-31         jsonlite_2.0.0      hms_1.1.3          
[40] listenv_0.9.1       glue_1.8.0          parallelly_1.43.0  
[43] nloptr_2.2.1        codetools_0.2-20    stringi_1.8.7      
[46] gtable_0.3.6        broom.mixed_0.2.9.6 pillar_1.10.2      
[49] furrr_0.3.1         htmltools_0.5.8.1   R6_2.6.1           
[52] Rdpack_2.6.4        evaluate_1.0.3      lattice_0.22-6     
[55] rbibutils_2.3       backports_1.5.0     Rcpp_1.0.14        
[58] nlme_3.1-168        mgcv_1.9-1          xfun_0.52          
[61] pkgconfig_2.0.3    
```
